...
class SubtitleManager:
    ...
    def update_text(self, segment_id: str, new_text: str, save_undo: bool = True):
        """Update segment text.

        When save_undo is True (default), saves a snapshot for undo.
        """
        if save_undo:
            self._save_state()
    ...
    def prevent_overlaps(self, adjust_words: bool = True):
        """Ensure segments do not overlap by adjusting boundaries to the midpoint.

        This is a simple post-processing pass for generated subtitles.
        It preserves the order and does not create gaps.
        """
        ...
        for i in range(1, len(self._segments)):
            ...
            curr_seg = self._segments[i]
            if curr_seg.start < prev_seg.end:
                overlap = prev_seg.end - curr_seg.start

                # Weight adjustment by duration (protect short segments)
                dur_prev = prev_seg.duration()
                dur_curr = curr_seg.duration()

                # If one is much shorter, let the longer one yield more.
                # Avoid division by zero
                if dur_prev + dur_curr > 0:
                    # Ratio of curr's length to total length
                    # If curr is short (small ratio), we want to move it LESS.
                    # So prev.end moves back by (1 - ratio) * overlap
                    # curr.start moves forward by ratio * overlap

                    # Example: prev=10s, curr=1s. Total=11s.
                    # ratio_curr = 1/11 ~= 0.09
                    # prev yields: 0.91 * overlap (moves back a lot)
                    # curr yields: 0.09 * overlap (moves forward a little)

                    ratio_curr = dur_curr / (dur_prev + dur_curr)

                    # Clamp ratio to avoid extreme shifts?
                    # Let's keep it pure proportional.

                    move_prev = (1.0 - ratio_curr) * overlap
                    move_curr = ratio_curr * overlap

                    mid = curr_seg.start + move_curr
                    # OR prev_seg.end - move_prev (should be same)

                    prev_seg.end = mid
                    curr_seg.start = mid
                else:
                    # Fallback 50/50
                    mid = curr_seg.start + (overlap / 2)
                    prev_seg.end = mid
                    curr_seg.start = mid

                if curr_seg.end < curr_seg.start:
                    curr_seg.end = curr_seg.start
                if prev_seg.start > prev_seg.end:
                    prev_seg.start = prev_seg.end

                if adjust_words:
                    if prev_seg.words:
                        for w in prev_seg.words:
                            if w.end > prev_seg.end:
                                w.end = prev_seg.end
                            if w.start > w.end:
                                w.start = w.end

                    if curr_seg.words:
                        for w in curr_seg.words:
                            if w.start < curr_seg.start:
                                w.start = curr_seg.start
                            if w.end < w.start:
                                w.end = w.start
    ...
    def split_segment(
        self, segment_id: str, split_time: float, save_undo: bool = True
    ) -> tuple[Optional[str], Optional[SubtitleSegment], Optional[SubtitleSegment]]:
        """
                ...
                if seg.start < split_time < seg.end:
                    ...
                    if current_words:
                        for word in current_words:
                            # If the split time falls within a word's duration,
                            # that word should start the second segment.
                            if word.end > split_time:
                                second_part_words.append(word)
                            else:
                                first_part_words.append(word)

                        # Clamp words to strict segment boundaries
                        if first_part_words:
                            # Last word of first part should not exceed split_time
                            last_w = first_part_words[-1]
                            if last_w.end > split_time:
                                last_w.end = split_time

                        if second_part_words:
                            # First word of second part should not start before split_time
                            first_w = second_part_words[0]
                            if first_w.start < split_time:
                                first_w.start = split_time
    ...
    def merge_segments(
        self, segment_ids: List[str], save_undo: bool = True
        ...
        merged_words = []
        for i in indices:
            merged_words.extend(self._segments[i].words)
        first.words = merged_words
    ...
    def export_metadata(self) -> List[dict[str, object]]:
