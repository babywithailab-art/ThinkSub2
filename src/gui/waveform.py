...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QMenu, QScrollBar
...
class WaveformWidget(QWidget):
    ...
    # Signals
    segment_clicked = Signal(str)  # Emitted when a segment is clicked
    split_requested = Signal(str, float)  # segment_id, time
    playback_started = Signal()
    playback_finished = Signal()
    region_changed = Signal(str, float, float)  # segment_id, new_start, new_end
    cursor_time_changed = Signal(float)
    scroll_cursor_time_changed = Signal(float)
    ...
    def __init__(self, parent=None, scrollbar_position: str = "bottom"):
        ...
        self._audio_end_time = 0.0  # Audio duration tracking

        # Track last scroll cursor time to prevent duplicate emits
        self._last_scroll_cursor_time: Optional[float] = None
    ...
    def _setup_ui(self):
        ...
        # Scroll Cursor Line (Blue, scroll sync) - MAKE DRAGGABLE
        self.scroll_cursor_line = pg.InfiniteLine(
            pos=0,
            angle=90,
            pen=pg.mkPen(color="#3a8dff", width=2, style=Qt.PenStyle.SolidLine),
            movable=True,  # Enable drag
            hoverPen=pg.mkPen(color="#6ab0ff", width=3),
        )
        ...
        # Connect scroll cursor drag signal
        self.scroll_cursor_line.sigDragged.connect(self._on_scroll_cursor_dragged)
    ...
    def extract_audio(self, start: float, end: float) -> Optional[np.ndarray]:
        ...
        try:
            # Optimization: Use cache if available (File Mode / Full Render)
            if self._full_audio_cache is not None:
                start_idx = int(start * self._sample_rate)
                end_idx = int(end * self._sample_rate)

                # Bounds check
                total_len = len(self._full_audio_cache)
                ...
                return self._full_audio_cache[start_idx:end_idx]

            # Fallback: Search chunks (Live Mode)
    ...
    def stop_playback(self):
        ...
        # Clean up timer to ensure is_playing() returns False
        if hasattr(self, "_anim_timer"):
            self._anim_timer.stop()
            self._anim_timer.deleteLater()
            delattr(self, "_anim_timer")
    ...
    def _stop_playback_cursor(self):
        was_active = hasattr(self, "_anim_timer") and self._anim_timer.isActive()
        if hasattr(self, "_anim_timer"):
            self._anim_timer.stop()
        ...
        # Only emit if we were actually playing to avoid signal spam
        if was_active:
    ...
    def start_time(self) -> float:
        """Get current visible start time (X-axis min)."""
        return self.plot_widget.viewRange()[0][0]

    @property
    def end_time(self) -> float:
        """Get current visible end time (X-axis max)."""
        return self.plot_widget.viewRange()[0][1]

    @property
    def cursor_time(self) -> float:
        """Get current blue line cursor time."""
    ...
    def add_segment_visual(self, segment):
        """Add a single segment visual."""
        # Init cache
        if not hasattr(self, "_segment_cache"):
            self._segment_cache = {}
        self._segment_cache[segment.id] = (segment.start, segment.end)

        # Check if already exists
        if segment.id in self._segment_items:
            self.update_segment_visual(segment)
            return

        # Basic color logic (Gray for Draft, Green for Confirmed implicit)
        # Using simple transparency
        color = QColor(100, 100, 100, 50)
        if hasattr(segment.status, "name") and segment.status.name != "DRAFT":
            color = QColor(0, 255, 0, 50)

        item = pg.LinearRegionItem(
            values=[segment.start, segment.end],
            brush=color,
            movable=self._edit_enabled,
            swapMode="block",
        )
        ...
        def _on_change():
            ...
            self.region_changed.emit(segment.id, region[0], region[1])

        item.sigRegionChangeFinished.connect(_on_change)

        # Capture Start of Interaction
        item.sigRegionChangeStarted.connect(
            lambda: self._on_region_change_started(segment.id, item)
        )

        # Handle Changing
        item.sigRegionChanged.connect(
            lambda: self._on_region_item_changing(segment.id, item)
        )

        self.plot_widget.addItem(item)
        self._segment_items[segment.id] = item

    def _ensure_drag_state_lazy(self, segment_id, item):
        """Recover drag state if signal was missed."""
        if not hasattr(self, "_segment_cache") or segment_id not in self._segment_cache:
            return False  # Cannot recover

        init_start, init_end = self._segment_cache[segment_id]

        # Re-calc walls
        others = []
        for sid, it in self._segment_items.items():
            if sid == segment_id:
                continue
            r = it.getRegion()
            others.append((r[0], r[1]))

        start, end = init_start, init_end

        left_wall = -1.0
        right_wall = float("inf")

        for o_start, o_end in others:
            if o_end <= start + 0.001:
                if o_end > left_wall:
                    left_wall = o_end
            if o_start >= end - 0.001:
                if o_start < right_wall:
                    right_wall = o_start

        self._drag_state = {
            "id": segment_id,
            "initial_region": (init_start, init_end),
            "initial_len": init_end - init_start,
            "left_wall": left_wall,
            "right_wall": right_wall,
        }
        print(
            f"[DEBUG] Lazy State Init for {segment_id}. Walls: {left_wall}/{right_wall}"
        )
        return True

    def _on_region_change_started(self, segment_id: str, item):
        """Capture initial state before drag."""
        print(f"[DEBUG] Start Change: {segment_id}")

        # Use Cached State for Start Position to find Walls consistently
        if hasattr(self, "_segment_cache") and segment_id in self._segment_cache:
            start, end = self._segment_cache[segment_id]
            init_region = (start, end)
        else:
            init_region = item.getRegion()
            start, end = init_region
        ...
        for o_start, o_end in others:
            # Check Left Wall (Max End <= Start)
            if o_end <= start + 0.001:
                if o_end > left_wall:
                    left_wall = o_end

            # Check Right Wall (Min Start >= End)
            if o_start >= end - 0.001:
                if o_start < right_wall:
                    right_wall = o_start
        ...
        self._drag_state = {
            ...
            "initial_len": init_region[1] - init_region[0],
            "left_wall": left_wall,
            "right_wall": right_wall,
        }

    def remove_segment_visual(self, segment_id: str):
        """Remove a segment visual."""
        if hasattr(self, "_segment_cache"):
            self._segment_cache.pop(segment_id, None)
        item = self._segment_items.pop(segment_id, None)
        if item:
            self.plot_widget.removeItem(item)

    def update_segment_visual(self, segment):
        """Update existing segment visual."""
        if not hasattr(self, "_segment_cache"):
            self._segment_cache = {}
        self._segment_cache[segment.id] = (segment.start, segment.end)

        item = self._segment_items.get(segment.id)
        if item:
            # Only update if different to avoid recursion loops if we are strict
            current = item.getRegion()
            if not (
                np.isclose(current[0], segment.start)
                and np.isclose(current[1], segment.end)
            ):
                item.setRegion([segment.start, segment.end])

    def _on_region_item_changing(self, segment_id: str, item):
        """Handle dragging with Hard Stop collision and Mode Enforcement."""
        if not self._snap_enabled:
            return

        if not hasattr(self, "_drag_state") or self._drag_state.get("id") != segment_id:
            print(
                f"[DEBUG] Missing/Mismatch State for {segment_id}, attempting lazy init."
            )
            if not self._ensure_drag_state_lazy(segment_id, item):
                return

        state = self._drag_state
        init_start, init_end = state["initial_region"]
        init_len = state["initial_len"]
        ...
        d_start = curr_start - init_start
        d_end = curr_end - init_end
        ...
        # Determine Mode
        # If both moved by roughly same amount -> Move
        is_move = abs(d_start - d_end) < 0.01

        if is_move:
            # MOVE: Force Length Preservation
            shift = d_start

            dist_left = init_start - left_wall
            dist_right = right_wall - init_end

            # Snap to walls
            if abs(shift + dist_left) < snap_dist:
                shift = -dist_left
            elif abs(dist_right - shift) < snap_dist:
                shift = dist_right

            # Hard Clamp to walls
            shift = max(-dist_left, min(dist_right, shift))

            final_start = init_start + shift
            final_end = init_start + shift + init_len

            # Mark dirty if we clamped/snapped OR if length was corrupted
            if (
                abs(final_start - curr_start) > 0.0001
                or abs(curr_len - init_len) > 0.001
            ):
                dirty = True

        else:
            # RESIZE: Lock static edge
            final_start, final_end = curr_start, curr_end

            # Check Left Edge Move
            if abs(d_start) > 0.001:
                # Force Right Fixed
                final_end = init_end

                # Clamp/Snap Left
                if final_start < left_wall:
                    final_start = left_wall
                    dirty = True
                elif final_start - left_wall < snap_dist and left_wall >= 0:
                    final_start = left_wall
                    dirty = True

                # Min Len
                if final_start > final_end - min_len:
                    final_start = final_end - min_len
                    dirty = True

            # Check Right Edge Move
            elif abs(d_end) > 0.001:
                # Force Left Fixed
                final_start = init_start

                # Clamp/Snap Right
                if final_end > right_wall:
                    final_end = right_wall
                    dirty = True
                elif right_wall - final_end < snap_dist and right_wall != float("inf"):
                    final_end = right_wall
                    dirty = True

                # Min Len
                if final_end < final_start + min_len:
                    final_end = final_start + min_len
                    dirty = True
        ...
        if dirty:
            ...
            item.setRegion([final_start, final_end])
    ...
    def clear(self):
        ...
        self._audio_store = []
        self._audio_start_time = 0.0
        self._audio_end_time = 0.0  # Added: prevent AttributeError
    ...
    def render_full_session(self):
        ...
        try:
            ...
            visible_seconds = (
                min(self._max_visible_seconds, end_time) if end_time else 0.0
            )
            if visible_seconds <= 0:
                visible_seconds = self._visible_seconds  # fallback

            # Update _audio_end_time for duration calculation
            self._audio_end_time = float(end_time) if end_time else 0.0
            self._dbg(f"[AUDIO] Updated _audio_end_time={self._audio_end_time:.1f}s")

            self._set_view_start(0.0, visible_seconds)
            self._sync_scrollbar(end_time if end_time else 0.0, visible_seconds)
    ...
    def _on_scrollbar_changed(self, value: int):
        ...
        sb = self._scrollbar
        self._dbg(f"[SCROLLBAR] value={value}/{sb.maximum()}, minimum={sb.minimum()}")

        visible_seconds = min(self._visible_seconds, self._max_visible_seconds)
        start = float(value) / 1000.0

        # Update view range WITHOUT updating scroll cursor
        if start is None or start < 0:
            start = 0.0
        end = start + visible_seconds

        # CLAMP: also prevent going past audio end
        audio_end = float(getattr(self, "_audio_end_time", 0.0) or 0.0)
        if end > audio_end and audio_end > 0:
            end = audio_end
            start = max(0.0, end - visible_seconds)

        self.plot_widget.setXRange(start, end, padding=0)
        self._dbg(f"[SCROLLBAR] view_range: start={start:.3f}s, end={end:.3f}s")

        # Update scrollbar position
        if self._scrollbar.isVisible():
            max_start = max(0.0, float(self._scrollbar.maximum()) / 1000.0)
            scroll_start = min(start, max_start)
            self._scrollbar.blockSignals(True)
            self._scrollbar.setValue(int(scroll_start * 1000))
            self._scrollbar.blockSignals(False)

        # Calculate center time and emit for sync
        center_t = start + visible_seconds / 2.0
        self._dbg(f"[SCROLLBAR] center_t={center_t:.3f}s, emit=True")
        self.set_scroll_cursor_pos(center_t, emit=True)

    def _on_scroll_cursor_dragged(self, line):
        """Handle blue line drag - emit signal for sync."""
        t = float(line.pos().x())
        self._dbg(f"[CURSOR_DRAG] t={t:.3f}s, emit=True")
        self.set_scroll_cursor_pos(t, emit=True)
    ...
    def _on_wheel_pan(self, event):
        ...
        # CLAMP: prevent negative start (which would cause center_t < 0)
        if new_start < 0:
            new_start = 0.0

        # If we have no scrollable range, do nothing
        audio_end = float(getattr(self, "_audio_end_time", 0.0) or 0.0)
        if audio_end <= 0:
            return

        visible_seconds = getattr(self, "_visible_seconds", 10.0)
        if audio_end <= visible_seconds:
            return

        # Update view range
        end = new_start + visible_seconds

        # CLAMP: also prevent going past audio end
        if end > audio_end and audio_end > 0:
            end = audio_end
            new_start = max(0.0, end - visible_seconds)

        self.plot_widget.setXRange(new_start, end, padding=0)

        # Ensure scrollbar is visible when scrollable
        if not self._scrollbar.isVisible() and audio_end > visible_seconds:
            self._sync_scrollbar(audio_end, visible_seconds)

        # Update scrollbar
        if self._scrollbar.isVisible():
            max_start = max(0.0, float(self._scrollbar.maximum()) / 1000.0)
            scroll_start = min(new_start, max_start)
            self._scrollbar.blockSignals(True)
            self._scrollbar.setValue(int(scroll_start * 1000))
            self._scrollbar.blockSignals(False)

        # Calculate center time and EMIT signal for sync
        center_t = new_start + visible_seconds / 2.0
        self._dbg(
            f"[WHEEL_PAN] delta={delta}, new_start={new_start:.3f}, center_t={center_t:.3f}s, emit=True"
        )
        self.set_scroll_cursor_pos(center_t, emit=True)

    def _set_view_start(self, start: float, end: Optional[float]) -> None:
        # Safety: ensure start and end are valid numbers
        if start is None or start < 0:
            start = 0.0
    ...
    def set_scroll_cursor_pos(self, time: float, emit: bool = True):
        # Prevent duplicate LOCAL emits at same time
        # BUT: always apply when emit=False (from master handler sync)
        if (
            emit
            and hasattr(self, "_last_scroll_cursor_time")
            and self._last_scroll_cursor_time == time
        ):
            self._dbg(f"[CURSOR] SKIP duplicate set_scroll_cursor_pos time={time}")
            return

        self._last_scroll_cursor_time = time
        self._dbg(f"[CURSOR] set_scroll_cursor_pos time={time:.3f}s, emit={emit}")

        if time < 0:
            self._dbg(f"[CURSOR] SKIP negative time={time:.3f}s")
            return
        self.scroll_cursor_line.setPos(time)
        self.scroll_cursor_line.setVisible(True)

        # DEBUG LOG: Cursor position updated
        self._dbg(f"[CURSOR_LINE] position updated to time={time:.3f}s")
        self.scroll_cursor_line.update()  # Force repaint (InfiniteLine has update(), not repaint())

        if emit:
            self._dbg(
                f"[CURSOR] emitting scroll_cursor_time_changed time={float(time):.3f}s"
            )
    ...
    def add_word_timestamps(self, segment_id: str, words: List[tuple]):
        ...
        lines = []
        # High-contrast yellow for readability on dark waveform
        ts_color = QColor(255, 230, 80, 230)
        ...
        for word in words:
            ...
            # Text
            # Anchor at top-left and place slightly lower to avoid clipping
            text_item = pg.TextItem(word_text, anchor=(0, 0))
            # HTML for small font
            text_item.setHtml(
                f'<div style="font-size: 8pt; color: #FFE650; background: rgba(0,0,0,0.55); padding: 0 2px; border-radius: 2px;">{word_text}</div>'
            )
            text_item.setPos(start_time, 0.8)  # Near top, avoid clipping
            text_item.setZValue(510)
    ...
    def refresh_segments(self, segments):
        ...
        # Cache latest segments for toggling word timestamps
    ...
    def _toggle_word_timestamps(self):
        ...
        # Rebuild word lines when toggling ON (in case they were never created)
        ...
        # Re-add from cached segments
        for seg in getattr(self, "_last_segments", []):
            if getattr(seg, "is_hidden", False):
                continue
            if seg.words:
                word_tuples = [
